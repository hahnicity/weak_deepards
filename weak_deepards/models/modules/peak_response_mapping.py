...
class PeakResponseMapping(nn.Sequential):
    ...
    def _median_filter(input):
        batch_size, num_channels, w = input.size()
        threshold, _ = torch.median(input.view(batch_size, num_channels, w), dim=2)
        return threshold.contiguous().view(batch_size, num_channels, 1)

    @staticmethod
    def _mean_filter(input):
        batch_size, num_channels, w = input.size()
        threshold = torch.mean(input.view(batch_size, num_channels, w), dim=2)
        return threshold.contiguous().view(batch_size, num_channels, 1)

    @staticmethod
    def _max_filter(input):
        batch_size, num_channels, w = input.size()
        threshold, _ = torch.max(input.view(batch_size, num_channels, w), dim=2)
        return threshold.contiguous().view(batch_size, num_channels, 1)
    ...
    def forward(self, input, class_threshold=0, peak_threshold=30, retrieval_cfg=None):
        assert input.dim() == 3, 'PeakResponseMapping layer only supports batch mode.'
        if self.inferencing:
            input.requires_grad_()

        # classification network forwarding. Utilizes base network
        ...
        if self.enable_peak_stimulation:
            ...
            if self.sub_pixel_locating_factor > 1:
                class_response_maps = F.upsample(class_response_maps, scale_factor=self.sub_pixel_locating_factor, mode='linear', align_corners=True)
