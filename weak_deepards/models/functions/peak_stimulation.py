...
class PeakStimulation(Function):
    ...
    def forward(ctx, input, return_aggregation, win_size, peak_filter):
        ...
        batch_size, num_channels, w = padded_maps.size()
        element_map = torch.arange(0, w).long().view(1, 1, w)[:, :, offset: -offset]
        element_map = element_map.to(input.device)
        # All this does is find the maximum indices for each M^c
    ...
    def backward(ctx, grad_peak_list, grad_output):
        ...
        batch_size, num_channels, _ = input.size()
        grad_input = peak_map * grad_output.view(batch_size, num_channels, 1)
