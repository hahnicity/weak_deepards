...
class TestApneaV2(object):
    def __init__(self):
        config_path = os.path.join(os.path.dirname(__file__), 'test_config.yml')
        with open(config_path) as conf:
            self.config = yaml.safe_load(conf)
        self.dataset = ApneaECGDataset(self.config['test_dataset_path'], 'train', 'main', 'intra', 'v2')
    ...
    def test_sunny_day(self):
        ...
        if (item[-1] == np.array([1, 0])).all():
            assert annos.symbol[1] == 'N'
        else:
            assert annos.symbol[1] == 'A'
        ...
        assert (((samp[3000:9000] - mu)).ravel() / std == item[2].ravel()).all()
    ...
    def test_seqs_properly_aligned(self):
        ...
        for i in range(len(self.dataset)):
            ...
            start = minutes[j] + 3000
            end = start + 6000
            assert (((samp[start:end] - mu)).ravel() / std == item[2].ravel()).all(), "record: {}, start: {}, end: {}".format(item[1], start, end)
            if annos.symbol[j+1] == 'A':
                assert (np.array([0, 1]) == item[-1]).all()
            if annos.symbol[j+1] == 'N':
                assert (np.array([1, 0]) == item[-1]).all()
            last_rec = item[1]


class TestApneaV1(object):
    def __init__(self):
        config_path = os.path.join(os.path.dirname(__file__), 'test_config.yml')
        with open(config_path) as conf:
            self.config = yaml.safe_load(conf)
        self.dataset = ApneaECGDataset(self.config['test_dataset_path'], 'train', 'main', 'intra', 'v1')
